...
void rowcolttests_c(double *x, int *fac, int nr, int nc, int no, int nt, 
                    ...
                    double *statistic, double *foldchange, double *df) {
    ...
    case 1:
        for(i=0; i<nt; i++) {
	    if (n[0][i] == 0) {
		df[i] = foldchange[i] = statistic[i] = R_NaReal;
		continue;
	    }
	    df[i]	 = n[0][i]-1;
	    ...
	    z            = ss[0][i];
	    foldchange[i]        = s[0][i];
	    statistic[i] =  factor * foldchange[i] / sqrt(z);
        }
        break;
    case 2:
	for(i=0; i<nt; i++) {
...
SEXP rowcolttests(SEXP _x, SEXP _fac, SEXP _nrgrp, SEXP _which, SEXP _na_rm)
{
  ...
  SEXP statistic, foldchange, df;  /* list elements for constructing 
                              the return value */
